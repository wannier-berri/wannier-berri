...
def num_cart_dim(key):
    ...
    if key in ["Ham", "overlap_up_down"]:
        return 0
    elif key in ["AA", "BB", "CC", "SS", "SOT", "SH", "OO", "rotAA", "dV_soc_wann_0_0", "dV_soc_wann_0_1", "dV_soc_wann_1_1"]:
        return 1
...
class System:
    ...
    def get_bandstructure(self,
        ...
        ----------
        dk : float
            the distance between k-points in the path in units of 1/Angstrom.
        parallel : bool
            if True and Ray is initialized, will calculate the band structure in parallel
        return_path : bool
            if True, will return the path along with the band structure
        path : Path, optional
            the path along which to calculate the band structure. If None, a path will be autogenerated
