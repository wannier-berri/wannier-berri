...
from .formula import Formula_ln, Matrix_ln , Matrix_GenDer_ln, FormulaProduct, FormulaSum
#####################################################
#####################################################
...
class MagMom(FormulaSum):
    " returns magnetic moment in units of Bohr magneton"

    def __init__(self,data_K,spin=True,orb=True,gender=0,**kwargs_formula):
        assert gender in (0,1)
        formulae = []
        coefs = []
        if orb:
            formulae.append( data_K.covariant('morb',gender = gender,**kwargs_formula))
            coefs.append(factor_morb)
        if spin:
