...
class FormulaProduct():
    ...
    def __init__(self,formula_list,subscripts=None,hermitian=False,name='unknown'):
        ...
        self.nonabelian = all([f.nonabelian for f in self.formulae])
    ...
    def __call__(self,ik,ib_in_start,ib_in_end,trace=True):#,Emax=None):
        ...
        if trace: 
            ...
            assert self.nonabelian , f"formula {self.name} can be called only as a trace"
            if self.hermitian:
...
class Formula():
    ...
    """
    def __init__(self,TRodd=False,Iodd=False,ndim=0,hermitian=True,name="unknown",nonabelian=False):
        self.name=name
        self.TRodd=TRodd
        self.Iodd=Iodd
        self.ndim=ndim
...
def _MatrixProductTerm_select(mat,ind,ib_in_start,ib_in_end,mat_T,add):
    ...
    if ind[1] in INNER_IND:
        ...
        if add:
            m1=np.concatenate((m1_T[:,0:ib_in_start],m1[:,ib_in_end:]),axis=1)
        else:
            m1=np.concatenate((m1[:,0:ib_in_start],m1[:,ib_in_end:]),axis=1)
    return m1
'''
