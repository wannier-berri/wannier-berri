...
class SymmetrizerSAWF:
    ...
    def from_dict(self, dic=None, return_obj=True, **kwargs):
        ...
        for prefix in ["T", "atommap", "rot_orb"]:
            keys_num = [int(k[len(prefix) + 1:]) for k in dic.keys() if k.startswith(prefix + "_")]
            keys = [f"{prefix}_{i}" for i in sorted(keys_num)]
    ...
    @classmethod
    def from_npz(cls, f_npz, allow_pickle=False):
