...
IRREP_IRREDUCIBLE_VERSION = "2.2.0"  # the version of irrep that supports irreducible band structure
...
class SymmetrizerSAWF:
    ...
    def from_irrep(self, bandstructure: BandStructure,
        ...
        if irreducible:
            ...
            import irrep
            if version.parse(irrep.__version__) < version.parse(IRREP_IRREDUCIBLE_VERSION):
                raise ImportError("The irreducible option requires irrep version >= 2.2.0, please update irrep")
            data = bandstructure.get_dmn(grid=grid, degen_thresh=degen_thresh, unitary=True, irreducible=True)
        else:
