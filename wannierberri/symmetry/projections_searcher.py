...
class EBRsearcher:
    ...
    * Not tested with spinor wavefunctions. It is recommended to search for
    ...
    def __init__(self,
        ...
        logger.debug(f"{self.irreps_per_projection_vectors=}")
    ...
    def set_irreps_D_wann(self, symmetrizer, max_multiplicity=None):
        ...
        logger.debug("detecting irreps generated by each D_wann block",
                     hihglight=True)
        ...
        logger.debug(f"{symmetrizer.D_wann_block_indices=} ")
        logger.debug(f"{len(symmetrizer.D_wann_blocks)=} ")
    ...
    def set_irreps_dft(self, symmetrizer,
                       ...
                       outer_min=-np.inf, outer_max=np.inf,
                       degen_thresh=1e-8):
        logger.debug("Detrimine all possible irreps", hihglight=True)
        logger.debug(" deternine the irreps in the DFT bands", hihglight=True)
        ...
        for ik in range(self.NKirr):
            ...
            logger.debug(f"ik= {ik} contains {nfrozen} frozen states\n" +
            ...
            logger.debug(f"Frozen states are represented by vector {vector_frozen}")
            logger.debug(f"Outer states are represented by vector {vector_outer}")
    ...
    def find_combinations(self, num_wann_min=0, num_wann_max=None, fixed=[]):
        ...
        logger.debug(f"From first point within outer window {len(combinations)} are valid")
        ...
        for ik in iksrt:
            ...
            logger.debug(f"From ik={ik}/{self.NKirr} with frozen and outer window {len(combinations)} are valid")
        return combinations
...
def find_combinations_max(vectors, vector_max, lfixed,
    ...
    if num_wann_max is None:
        ...
        logger.info("unlimited number of wannier functions")
    else:
