...
class WorkflowQE:
    ...
    def wannierise_wberri(self, enforce=False, kwargs_system={}, kwargs_window={}, files=["mmn", "amn", "eig", "win"], **kwargs):
        if self.flags.check('wannierise_wberri') and not enforce:
            return
        w90data = Wannier90data.from_w90_files(seedname=self.prefix, files=files)
    ...
    def calc_bands_qe(self, kdensity, enforce=False, run=True, **kargs):
        ...
        self.path_qe = Path(real_lattice=self.atoms.get_cell(), nodes=self.k_nodes, length=kdensity)
...
def get_wannier_band_structure(system, k_nodes, length=1000, npar=0, parallel=True):
    ...
    """
    path = Path(system=system, nodes=k_nodes, length=length)
