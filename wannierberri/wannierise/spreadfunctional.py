...
class SpreadFunctional:
    ...
    def __call__(self, U):
        ...
        rn = np.array(sum(self.w[ib] * phinkb[ik, ib][:, None] * self.bk[ib]  for ib in range(NNB) for ik in range(NK)))
        rm2sum = np.linalg.norm(rn)**2
        # print ("wannier centers\n", rn)
        Omega_D = sum(self.w[ib] * (phinkb[ik, ib, n] - self.bk[ib] @ rn[n])**2 for n in range(NW) for ib in range(NNB) for ik in range(NK))
