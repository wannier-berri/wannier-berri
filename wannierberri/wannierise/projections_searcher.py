...
class EBRsearcher:
    ...
    def __init__(self, spacegroup,
        ...
        print (f"num_wann_per_projection={self.num_wann_per_projection}")
        ...
        for ik in range(self.NKirr):
            ...
            self.irreps_frozen_vectors.append(np.ceil(np.round(vector_frozen,3)).astype(int))
            self.irreps_outer_vectors.append(np.floor(np.round(vector_outer,3)).astype(int))
            ...
            if nband_frozen_irreps < nfrozen:
                raise RuntimeError("Some frozen bands are not covered by irreps onduced by trial projections"
