...
class Der2Dcov(Formula_ln):
    ...
    def ln(self, ik, inn, out):
        ...
        tmp = cached_einsum("lpbe,pnd->lnbde", dVll, Dln)
        summ += tmp + tmp.swapaxes(2, 3)
        tmp = cached_einsum("lpe,pnbd->lnbde", Vll, dDln)
        summ += tmp + tmp.swapaxes(3, 4) + tmp.transpose(0, 1, 4, 2, 3)
        tmp = -cached_einsum("lmde,mnb->lnbde", dDln, Vnn)
        summ += tmp + tmp.swapaxes(2, 3) + tmp.transpose(0, 1, 3, 4, 2)
        tmp = -cached_einsum("lmb,mnde->lnbde", Dln, dVnn)
        summ += tmp + tmp.swapaxes(2, 3)
