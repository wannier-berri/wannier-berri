...
def pertOmega(data_K,pertS,pertScomma,op=None,ed=None):
        # pertScomma - the comma derivative of perturbation, with derivative being the first cartesian index
        ndimPert  = pertS.ndim - 3 
        print ( "perturbation is {} dimensiona; ".format(ndimPert))
        A    = data_K.A_Hbar[op:ed]
        D    = data_K.D_H[op:ed]
        V    = data_K.V_H[op:ed]
        ...
        S    = pertS     [op:ed,:,:,None]
        Sc   = pertScomma[op:ed]
        dEi  = data_K.dEig_inv[op:ed]
        T    = -S*dEi.reshape(dEi.shape+(1,)*(ndimPert) ) [:,:,:,None]
        Acal= (-(A+1j*D)*data_K.dEig_inv[op:ed,:,:,None])
        del dEi

        # Now add the dimentsions of the perturbation
        newshape=A.shape+(1,)*ndimPert
...
def pertOmegaZeemanSpin(data_K,op=None,ed=None,Bdirection=None):
        pertS     = data_K.S_H
        pertScomma = data_K.delS_H.transpose(0,1,2,4,3)  # transpose, because the derivative is the last index, but should be the first
        ...
        return pertOmega(data_K,pertS,pertScomma,op=op,ed=ed)
