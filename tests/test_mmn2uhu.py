...
def test_mmn2uhu(create_files_Fe_W90):
    ...
    readnnkp = False
    wandata2 = WannierData.from_w90_files(seedname=os.path.join(data_dir, 'Fe'),
                                          readnnkp=readnnkp,
                                          files=['eig', 'win', 'mmn', 'spn', 'uhu', 'uiu', 'siu', 'shu'])
    wandata1 = WannierData.from_w90_files(seedname=os.path.join(data_dir, 'Fe'),
                                          readnnkp=readnnkp,
                                          files=['eig', 'win', 'mmn', 'spn'])
    wandata1.set_uHu_from_mmn_eig()
    wandata1.set_uIu_from_mmn()
    wandata1.set_sIu_from_mmn_spn()
    wandata1.set_sHu_from_mmn_eig_spn()
    for file in ['uhu', 'uiu', 'siu', 'shu']:
        data1 = wandata1.get_file(file).data
        data2 = wandata2.get_file(file).data
