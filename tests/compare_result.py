...
def read_energyresult_dat(filename,cmplx=False):
    ...
    if cmplx:
        assert n_data % 2 == 0 , "found odd n_data={} (N_energies={})".format(n_data, N_energies)
...
def compare_energyresult(rootdir):
    ...
    def _inner(fout_name, suffix, adpt_num_iter,cmplx=False,suffix_ref=None):
        ...
        for i_iter in range(adpt_num_iter+1):
            ...
            E_titles, data_energy, data, data_smooth = read_energyresult_dat(filename,cmplx=cmplx)
            path_filename_ref = os.path.join(rootdir, 'reference', filename_ref)
            E_titles_ref, data_energy_ref, data_ref, data_smooth_ref = read_energyresult_dat(path_filename_ref,cmplx=cmplx)
